...
- [x] Modify SurrealDB semantic search for agent-specific filtering (AC: 1)
  - [x] Update `semantic_search()` to accept optional `agent_id` parameter
  - [x] Implement graph traversal from subjective agent perspective
  - [x] Test filtering logic for agent-specific beliefs
- [x] Implement universal facts integration (AC: 2)
  - [x] Combine subjective and universal fact queries in UNION operation
  - [x] Ensure system facts are accessible via `agent:system->BELIEVES->fact` path
  - [x] Test query composition and execution
- [x] Update BaseAgent with automatic agent ID injection (AC: 3)
  - [x] Modify `recall_memory()` to automatically pass `self.config.name` to search tools
  - [x] Ensure tool abstraction hides agent ID from LLM
  - [x] Test agent identification and context passing
- [x] Update MemoryConsolidator for subjective fact management (AC: 4)
  - [x] Link "User preference" facts to `agent:system` by default for universal access
  - [x] Link "Agent opinion" facts to specific agent for subjective perspectives
  - [x] Test fact consolidation and belief system consistency
- [x] Create comprehensive subjective retrieval tests (AC: 5)
  - [x] Unit tests for agent-specific filtering
  - [x] Integration tests for subjective vs universal fact retrieval
  - [x] Tests for memory consolidation and belief consistency
  - [x] Edge case tests for conflicting beliefs and resolution
