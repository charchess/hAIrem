...
class SurrealDbClient:
    ...
    async def insert_graph_memory(self, fact_data: dict[str, Any]):
        ...
        """
        if not self.client or not SURREAL_AVAILABLE:
            return
        ...
        try:
            ...
            await self._call(
                "query",
                f"INSERT INTO subject (id, name) VALUES ({sid}, '{subject_name}') ON DUPLICATE KEY UPDATE name = '{subject_name}';",
            )
            await self._call(
                "query",
                f"INSERT INTO subject (id, name) VALUES ({aid}, '{agent_name}') ON DUPLICATE KEY UPDATE name = '{agent_name}';",
            )

            # 2. Create Fact node
            fact_res = await self._call("create", "fact", {"content": fact_content, "embedding": embedding})
            if not fact_res:
                return
            ...
            await self._call(
                "query",
                f"RELATE {aid}->BELIEVES->{fid} SET confidence = {confidence}, strength = 1.0, last_accessed = time::now();",
            )
            ...
            logger.info(
                f"GRAPH_MEMORY: Linked {agent_name} -> BELIEVES -> Fact('{fact_content[:30]}...') ABOUT {subject_name}"
            )
    ...
    async def semantic_search(
        self, embedding: list[float], agent_id: str | None = None, limit: int = 5
    ) -> list[dict[str, Any]]:
        """
    ...
    async def apply_decay_to_all_memories(self, decay_rate: float, threshold: float = 0.1):
        ...
        Args:
            decay_rate: The decay rate (e.g., 0.05 for 5% decay)
        ...
        try:
            ...
            # Use time-based decay with last_reinforced field
            decay_query = f"""
            UPDATE BELIEVES 
            SET strength = strength * math::pow({decay_rate}, time::now() - last_reinforced),
    ...
    async def merge_or_override_fact(self, old_fact_id: str, new_fact_data: dict[str, Any], resolution: dict[str, Any]):
        ...
        try:
            ...
            if action == "OVERRIDE":
                ...
                await self._call(
                    "query",
                    f"UPDATE {old_fact_id} SET content = '{new_content}', embedding = {new_fact_data['embedding']};",
                )
    ...
    async def get_messages(self, limit: int = 50) -> list[dict[str, Any]]:
        ...
        try:
            ...
            res = await self._call(
                "query",
                f"SELECT * FROM (SELECT * FROM messages ORDER BY timestamp DESC LIMIT {limit}) ORDER BY timestamp ASC;",
            )
