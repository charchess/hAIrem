...
try:
    ...
    logging.warning(f"Whisper dependencies (torch/faster-whisper) not available: {e}")
...
class WhisperService:
    ...
    async def initialize(self, model_size="base", device="auto"):
        ...
        if not TORCH_AVAILABLE:
            logger.error("Cannot initialize Whisper: torch/faster-whisper not installed.")
            return False
        try:
            ...
            self.model = faster_whisper.WhisperModel(
                model_size_or_path=model_size, device=self.device, compute_type="float32"
            )
        ...
        while self.is_processing:
            try:
                ...
                try:
                    ...
                    session_id, audio_data, room_id = item  # Expecting 3 parts

                    # Process synchronously in this thread
                    self._process_audio_chunk_sync(session_id, audio_data, room_id)
    ...
    async def _send_to_llm(self, transcription_data: Dict[str, Any]):
        ...
        try:
            message = HLinkMessage(
                ...
                sender=Sender(agent_id="user", role="user"),  # Use 'user' directly for cleaner routing
                ...
                payload=Payload(
                    ...
                    room_id=transcription_data.get("room_id", "Salon"),
                ),
            )
    ...
    def _process_audio_chunk_sync(self, session_id: str, audio_bytes: bytes, room_id: str = "Salon"):
        ...
        try:
            ...
            # Transcribe
            result = self.model.transcribe(audio_data, language="fr", task="transcribe", vad_filter=True)
    ...
    async def _send_error_update(self, session_id: str, error: str):
        ...
        try:
            error_data = {"session_id": session_id, "error": error, "timestamp": time.time()}

            message = HLinkMessage(
                type=MessageType.TRANSCRIPTION_ERROR,
                sender=Sender(agent_id="whisper_pipeline", role="transcriber"),
                recipient=Recipient(target="a2ui"),
                payload=Payload(content="Transcription error", session_id=session_id, error=error),
            )
    ...
    async def process_audio_request(self, session_id: str, audio_data: bytes, room_id: str = "Salon"):
        """Public interface for audio processing requests."""
        try:
            # Put raw bytes in queue with room_id
            self.transcription_queue.put((session_id, audio_data, room_id))
    ...
    async def start_session(self, session_id: str):
        ...
        return {"session_id": session_id, "status": "started", "device": self.device, "model_size": self.model_size}
...
async def handle_whisper_request(websocket, message: Dict[str, Any], redis_client):
    ...
    try:
        ...
        if message_type == "whisper_session_start":
            session_data = await whisper_service.start_session(message.get("payload", {}).get("session_id", "default"))
            await websocket.send_text(json.dumps(session_data))

        elif message_type == "whisper_session_end":
            session_data = await whisper_service.end_session(message.get("payload", {}).get("session_id", "default"))
            await websocket.send_text(json.dumps(session_data))
            ...
            response = HLinkMessage(
                ...
                recipient=Recipient(target="a2ui"),
                payload=Payload(content=status, status="ready" if status["initialized"] else "initializing"),
            )
