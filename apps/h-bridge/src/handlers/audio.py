...
class AudioProcessor:
    ...
    async def process_audio_message(self, websocket: WebSocket, message: Dict[str, Any]):
        ...
        try:
            ...
            audio_data = payload.get("content")
            audio_format = payload.get("format", "webm")
            sample_rate = payload.get("sample_rate", 16000)
            duration = payload.get("duration")
            room_id = payload.get("room_id", "Salon")
            ...
            # Decode base64 audio
            audio_bytes = (
                base64.base64decode(audio_data) if hasattr(base64, "base64decode") else base64.b64decode(audio_data)
            )
            ...
            error_message = HLinkMessage(
                ...
                sender=Sender(agent_id="audio_processor", role="system"),
                recipient=Recipient(target="a2ui"),
                payload=Payload(content=f"Audio processing error: {str(e)}", error_type="processing_error"),
            )
    ...
    async def handle_stt_result(self, stt_text: str, websocket: WebSocket):
        ...
        try:
            ...
            user_message = HLinkMessage(
                ...
                payload=Payload(content=stt_text, format="text", source="voice_input"),
            )
            ...
            ack_message = HLinkMessage(
                ...
                sender=Sender(agent_id="audio_processor", role="system"),
                recipient=Recipient(target="a2ui"),
                payload=Payload(content=f"Transcription complete: {len(stt_text)} characters", status="transcribed"),
            )
    ...
    async def handle_session_management(self, websocket: WebSocket, action: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if action == "start_session":
                self.active_sessions[session_id] = {"started_at": asyncio.get_event_loop().time(), "status": "active"}

                response = HLinkMessage(
                    type=MessageType.AUDIO_SESSION_RESPONSE,
                    sender=Sender(agent_id="audio_processor", role="system"),
                    recipient=Recipient(target="a2ui"),
                    payload=Payload(content="Audio session started", session_id=session_id, status="active"),
                )
                ...
                response = HLinkMessage(
                    ...
                    sender=Sender(agent_id="audio_processor", role="system"),
                    recipient=Recipient(target="a2ui"),
                    payload=Payload(content=f"Audio session status: {status}", session_id=session_id, status=status),
                )
...
async def handle_audio_message(websocket: WebSocket, message: Dict[str, Any], redis_client):
    ...
    try:
        ...
        error_message = HLinkMessage(
            ...
            sender=Sender(agent_id="audio_processor", role="system"),
            recipient=Recipient(target="a2ui"),
            payload=Payload(content=f"Audio processing error: {str(e)}", error_type="message_handling_error"),
        )
