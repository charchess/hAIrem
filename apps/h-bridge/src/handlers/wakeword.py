...
class WakeWordEngine:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.is_active = True
            self.background_thread = threading.Thread(target=self.background_processing_loop, daemon=True)
    ...
    async def on_wake_word_detected(self, confidence: float):
        ...
        try:
            ...
            trigger_message = HLinkMessage(
                ...
                payload=Payload(content="Auto-start recording", trigger_source="wake_word", confidence=confidence),
            )
    ...
    try:
        ...
        if message_type == "wakeword_status_request":
            ...
            status_message = HLinkMessage(
                ...
                payload=Payload(content="Wake word engine status", active=engine.is_active),
            )
            ...
            response_message = HLinkMessage(
                ...
                sender=Sender(agent_id="wakeword_engine", role="system"),
                recipient=Recipient(target="a2ui"),
                payload=Payload(content="Wake word engine enabled" if success else "Failed to enable", enabled=success),
            )
            ...
            disable_message = HLinkMessage(
                ...
                sender=Sender(agent_id="wakeword_engine", role="system"),
                recipient=Recipient(target="a2ui"),
                payload=Payload(content="Wake word engine disabled", enabled=False),
            )
...
def extract_audio_features(audio_data: np.ndarray, sample_rate: int = 16000):
    ...
    try:
        ...
        # Zero crossing rate
        zcr = np.mean(librosa.feature.zero_crossing_rate(audio_data, frame_length=2048, hop_length=512))
        ...
        return {"energy": energy, "zcr": zcr, "spectral_centroid": spectral_centroid}
    except Exception as e:
        logger.error(f"Feature extraction failed: {e}")
        return {"energy": 0.0, "zcr": 0.0, "spectral_centroid": 0.0}
