...
class MemoryIsolationService:
    ...
    async def create_user(
        self, user_id: str, email: str = "", name: str = "", session_id: Optional[str] = None
    ) -> dict[str, Any]:
        """Create a new user."""
        user_key = f"user:{user_id}"

        user_data = {"id": user_id, "email": email, "name": name, "created_at": str(uuid4())}
    ...
    async def store_memory(
        ...
        logger.info(
            f"Stored memory for user {effective_user_id}, session {effective_session_id[:8]}..., data = {memory_data}"
        )
    ...
    async def get_memory(self, user_id: str, session_id: Optional[str] = None) -> dict[str, Any]:
        ...
        if not is_valid:
            ...
            if error and "does not match" in error.lower():
            ...
            for sid in session_keys:
                if sid:
                    ...
                    if memory_data:
                        ...
                        memories.append({"sessionId": sid, "data": memory_data})
