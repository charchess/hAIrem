...
class VisualBible:
    ...
    def load_all(self):
        ...
        # 4. Load Personas from agent folders
        if os.path.exists(agents_dir):
            for agent_id in os.listdir(agents_dir):
                ...
                if os.path.exists(persona_path):
                    ...
                    data["_base_path"] = agent_dir
                    self.personas[agent_id.lower()] = data
        
        logger.info(f"BIBLE: Loaded {len(self.poses)} poses, {len(self.attitudes)} attitudes, {len(self.personas)} personas.")

    def get_persona_data(self, agent_id):
        self.load_all() # Hot reload
    ...
    def get_prompt_parts(self, agent_id, pose_key="neutral", attitude_key="standing"):
        ...
        res = {
            ...
            "global_style": self.style.get("character_style") or self.style.get("global_style", ""),
            "engine_directives": self.style.get("engine_directives", ""),
            "post_process": self.style.get("post_process_constraints", "")
        }
        return res

bible = VisualBible()

def build_prompt(agent_id, description="", pose="neutral", attitude="standing", outfit=None, **kwargs):
    ...
    prompt = (
        f"{parts['character_desc']} {outfit_part} {action_part} "
