...
class VisualImaginationService:
    ...
    async def generate_for_agent(self, agent_id: str, prompt: str, **kwargs: Any) -> str:
        ...
        # STORY 25.5: Fetch current agent state (outfit, location) from DB for prompt injection
        if "outfit" not in kwargs or "location" not in kwargs:
            try:
                states = await self.asset_manager.db.get_agent_state(agent_id)
                for s in states:
                    rel = s.get("relation")
                    if rel == "WEARS" and "outfit" not in kwargs:
                        kwargs["outfit"] = s.get("description", s.get("name"))
                    elif rel == "IS_IN" and "location" not in kwargs:
                        kwargs["location"] = s.get("description", s.get("name"))
            except Exception as e:
                logger.warning(f"VISUAL_SERVICE: Failed to fetch agent state for {agent_id}: {e}")
    ...
    async def generate_and_index(
        ...
        try:
            ...
            if asset_type == "pose":
                await self._remove_background(local_path)

            asset_uri, asset_id = await self.index_generated_asset(
                local_path, agent_id, prompt, tags, reference_image_used=ref_path
            )
    ...
    async def notify_visual_asset(self, url: str, prompt: str, agent_id: str, asset_type: str):
        ...
        msg = HLinkMessage(
            ...
            recipient=Recipient(target="broadcast"),
            payload=Payload(
                content={"url": public_url, "alt_text": prompt, "agent_id": agent_id, "asset_type": asset_type}
            ),
        )
        await self.redis.publish("broadcast", msg)
