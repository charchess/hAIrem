    ...
    def emit(self, record):
        ...
        try:
            ...
            asyncio.run_coroutine_threadsafe(
                self.redis.publish_event("system_stream", h_msg.model_dump(mode="json")), self.loop
            )
        except Exception:
            pass
        finally:
...
class HaremOrchestrator:
    def __init__(self):
        ...
        self.wakeword_service: Optional[WakewordService] = None
    ...
    async def setup(self):
        ...
        self.visual_service = VisualImaginationService(provider, asset_manager, self.llm, self.redis)
        ...
        self.mobile_location_service = MobileLocationService(
            self.location_service, exterior_service=self.exterior_service
        )
        await self.mobile_location_service.initialize()

        # Wakeword Service (Story 14.2)
        import json

        wakeword_config = os.getenv("WAKEWORD_CONFIG")
        if wakeword_config:
            try:
                config = json.loads(wakeword_config)
            except:
                config = {"wakeword": {"wakeword": "hey_lisa", "threshold": 0.5}}
        else:
            config = {"wakeword": {"wakeword": "hey_lisa", "threshold": 0.5}}

        self.wakeword_service = WakewordService(config=config, redis_client=self.redis, surreal_client=self.surreal)
        await self.wakeword_service.initialize()
        await self.wakeword_service.start()
        ...
        self.plugin_loader = PluginLoader(
            os.getenv("AGENTS_PATH", "/app/agents"), self.agent_registry, self.redis, self.llm, self.surreal
        )
    ...
    async def handle_system_message(self, data: dict):
        ...
        try:
            ...
            # Wakeword Detection (Story 14.2)
            if msg.type == "wakeword.detected":
                logger.info("ROUTER: Wakeword detected - activating voice interaction")
                # Publish activation event for voice processing
                activation_msg = HLinkMessage(
                    type="voice.activated",
                    sender=Sender(agent_id="wakeword", role="system"),
                    recipient=Recipient(target="broadcast"),
                    payload=Payload(content=msg.payload.content),
                )
                await self.redis.publish_event("system_stream", activation_msg.model_dump(mode="json"))
                return
            ...
            if msg.type in [
                MessageType.NARRATIVE_TEXT,
                MessageType.EXPERT_RESPONSE,
                MessageType.VISUAL_ASSET,
                ...
                if target != "broadcast" and target != "all":
                    ...
                    if agent and not agent.is_active:
                        unavailable_msg = HLinkMessage(
                            ...
                            recipient=Recipient(target=msg.sender.agent_id),
                            payload=Payload(
                                content=f"Désolée, l'agent '{target}' est actuellement indisponible. Veuillez réessayer plus tard."
                            ),
                        )
    ...
    async def message_router(self):
        """Central message routing using Redis Streams."""
        logger.info("ROUTER: Listening on 'system_stream' group 'h-core'...")
        await self.redis.listen_stream(
            "system_stream", "h-core", os.getenv("HOSTNAME", "core-1"), self.handle_system_message
        )
