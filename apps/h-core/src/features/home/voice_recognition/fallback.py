...
class VoiceRecognitionFallback:
    ...
    def create_anonymous_session(self, session_id: str) -> SessionUser:
        session_user = SessionUser(session_id=session_id, user_id=None, identified_at=None, is_anonymous=True)
        self._session_users[session_id] = session_user
        logger.info(f"Created anonymous session: {session_id}")
        return session_user

    def assign_user_to_session(self, session_id: str, user_id: str, user_name: Optional[str] = None) -> SessionUser:
        session_user = SessionUser(
            session_id=session_id, user_id=user_id, identified_at=datetime.utcnow(), is_anonymous=False
        )
        self._session_users[session_id] = session_user
    ...
    def handle_unidentified_voice(
        self, session_id: str, identification_result: VoiceIdentificationResult
    ) -> SessionUser:
        ...
        if identification_result.identified:
            return self.assign_user_to_session(
                session_id, identification_result.user_id or "", identification_result.user_name
            )

        return self.create_anonymous_session(session_id)

    def manual_identify(self, session_id: str, user_id: str, user_name: Optional[str] = None) -> SessionUser:
