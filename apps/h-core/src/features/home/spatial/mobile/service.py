...
class ThrottleTracker:
    ...
    async def is_allowed(self, client_id: str) -> bool:
        async with self._lock:
            ...
            self._update_count[client_id] = [t for t in self._update_count[client_id] if now - t < timedelta(minutes=1)]
...
class MobileLocationService:
    def __init__(
        ...
        throttle_config: Optional[LocationThrottleConfig] = None,
    ...
    async def handle_location_update(self, client_id: str, location: MobileLocationUpdate) -> dict:
        ...
        if self.exterior_service and location.latitude is not None:
            ...
            logger.info(
                f"MobileLocationService: Spatial context for {location.agent_id}: exterior={spatial_context.is_exterior}, theme={spatial_context.exterior_theme}"
            )
        ...
        if location.room_id:
            ...
            if hasattr(self.location_service, "track_ambiguous_location"):
                tracked = await self.location_service.track_ambiguous_location(
                    location.agent_id, location.room_id, confidence_level, confidence_reason
                )
                tracked_confidence = tracked.get("location", {}).get("confidence")
            ...
            logger.info(
                f"MobileLocationService: Updated location for agent {location.agent_id} to room {location.room_id}"
            )
            return {
                "success": True,
                "location": {
    ...
    async def handle_disconnect(self, client_id: str) -> Optional[MobileClientInfo]:
        async with self._lock:
            ...
            if client_info:
                ...
                logger.info(
                    f"MobileLocationService: Client {client_id} disconnected, last location preserved: room={client_info.last_room_id}"
                )
            return client_info
