...
def test_addressing_check(mock_agent):
    ...
    assert mock_agent._check_addressing("Hello everyone") is None
    ...
    assert mock_agent._check_addressing("Salut Lisa et Renarde") is True # Mentions both, both should respond? Yes for now.
...
async def test_log_level_filtering():
    ...
    for _ in range(10):
        if mock_redis.publish.called: break
        await asyncio.sleep(0.01)
