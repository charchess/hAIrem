...
async def test_conflict_resolver_logic():
    ...
    mock_llm.get_completion.return_value = json.dumps({
        "is_conflict": True,
        "resolution": "User no longer likes tea",
        "action": "OVERRIDE"
    })
...
async def test_consolidator_triggers_conflict_check():
    ...
    mock_llm.get_completion.side_effect = [
        json.dumps([{"fact": "User hates tea", "subject": "user", "confidence": 1.0}]), # Extraction
        json.dumps({"is_conflict": True, "resolution": "User changed mind about tea", "action": "OVERRIDE"}) # Resolution
    ]
