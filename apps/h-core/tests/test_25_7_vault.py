...
async def test_vault_service_save_and_get(mock_db):
    ...
    success = await service.save_item("lisa", "cool_dress", "visual_asset:abc", "garment")
    assert success is True
    assert mock_db._call.called

    # Mocking successful get
    mock_db._call.return_value = [{"result": [{"name": "cool_dress", "category": "garment", "asset": {"url": "file:///tmp/cool.png"}}]}]
...
async def test_command_handler_vault_hit_outfit(mock_redis, mock_visual, mock_db):
    ...
    # Mock vault hit
    mock_db._call.return_value = [{"result": [{"name": "red_dress", "category": "garment", "asset": {"url": "file:///tmp/red.png"}}]}]
...
async def test_command_handler_vault_list(mock_redis, mock_visual, mock_db):
    ...
    # Mock vault contents
    mock_db._call.return_value = [{"result": [
        {"name": "dress1", "category": "garment"},
        {"name": "beach", "category": "background"}
    ]}]
