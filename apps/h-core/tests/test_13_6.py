...
async def test_update_agent_state_logic():
    ...
    # Verify Invalidation
    assert any("DELETE WEARS WHERE in = subject:`lisa`" in c for c in calls)
    
    # Verify Target Node Update
    assert any("UPDATE outfit:`red_dress` SET description = 'A beautiful red dress', name = 'red_dress'" in c for c in calls)
    
    # Verify New Relation
    assert any("RELATE subject:`lisa`->WEARS->outfit:`red_dress` SET timestamp = time::now()" in c for c in calls)
...
async def test_get_agent_state_query():
    ...
    assert "FROM WEARS, IS_IN" in query
    assert "WHERE in = subject:`lisa`" in query
...
async def test_recall_memory_includes_live_facts():
    ...
    assert "CURRENTLY WEARING: A beautiful red dress (red_dress)" in result
    assert "CURRENT LOCATION: bedroom" in result
