...
from src.features.home.voice_recognition.embedding import VoiceEmbeddingExtractor
from src.features.home.voice_recognition.matcher import VoiceMatcher
from src.features.home.voice_recognition.fallback import VoiceRecognitionFallback
from src.features.home.voice_recognition.service import VoiceRecognitionService
from src.features.home.voice_recognition.models import (
    VoiceProfile,
...
class TestFallbackToAnonymousSession:
    ...
    def test_handle_unidentified_voice_assigns_user_when_identified(self):
        fallback = VoiceRecognitionFallback()
        result = VoiceIdentificationResult(identified=True, user_id="user123", user_name="Test User", confidence=0.9)
...
class TestVoiceRecognitionService:
    ...
    async def test_enroll_voice(self):
        ...
        profile = await service.enroll_voice(
            VoiceEnrollmentRequest(user_id="user123", name="Test User", audio_data=audio_data)
        )
    ...
    async def test_identify_voice_success(self):
        ...
        mock_repository.get_all_profiles = AsyncMock(
            return_value=[{"user_id": "user123", "name": "Test User", "embedding": [1.0] * 80}]
        )
        ...
        result = await service.identify_voice(
            VoiceIdentificationRequest(session_id="session123", audio_data=audio_data)
        )
    ...
    async def test_identify_voice_with_empty_audio(self):
        ...
        result = await service.identify_voice(VoiceIdentificationRequest(session_id="session123", audio_data=b""))
