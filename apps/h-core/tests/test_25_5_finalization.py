...
def mock_surreal():
    surreal = MagicMock()
    surreal.get_agent_state = AsyncMock(
        return_value=[
...
async def test_visual_raw_prompt_broadcast(mock_redis, mock_visual_provider, mock_asset_manager, mock_llm):
    ...
    broadcast_calls = [
        call for call in mock_redis.publish.call_args_list if call[0][1].type == MessageType.VISUAL_RAW_PROMPT
    ]
...
async def test_burning_memory_injection(mock_redis, mock_llm, mock_surreal):
    ...
    assert "CURRENTLY WEARS: A beautiful red dress" in system_msg["content"]
    assert "CURRENTLY IS IN: The sunny beach" in system_msg["content"]
