...
- [x] Task 1: Setup environnement MeloTTS (Docker profile `tts` dans docker-compose.yml)
- [x] Task 2: Intégration MeloTTS dans h-bridge (`tts_melotts.py` — client HTTP sync)
- [x] Task 3: Setup environnement OpenVoice v2 (même container melotts-openvoice)
- [ ] Task 4: Création des échantillons de référence pour Lisa, Electra, Renarde (fichiers WAV manuels requis)
- [x] Task 5: Intégration OpenVoice pour le clonage (`voice_openvoice.py` — zero-shot cloning)
- [x] Task 6: Streaming WebSocket audio — chunks 64KB dans `tts.py` melo engine block
- [x] Task 7: Mise à jour des `persona.yaml` avec `voice_config` (language, speed, tone, voice_ref)
- [x] Task 8: `tts_service.py` — orchestrateur MeloTTS → OpenVoice + file cache
- [x] Task 9: Tests (9/9 pass) — `test_tts_melotts.py` + `test_voice_openvoice.py`
