...
- [x] Task 1: Définir le schéma SurQL (AC: 3, 4, 5, 6)
  - [x] Créer `apps/h-core/src/infrastructure/graph_schema.surql`.
  - [x] Implémenter les `DEFINE TABLE` et `DEFINE FIELD` pour tous les noeuds et arêtes.
  - [x] Ajouter les index de performance sur les arêtes `BELIEVES`.
- [x] Task 2: Mettre à jour le client SurrealDB (AC: 4, 5)
  - [x] Modifier `apps/h-core/src/infrastructure/surrealdb.py` pour supporter les requêtes `RELATE`.
  - [x] Implémenter une méthode `insert_graph_memory(fact_data)` gérant la création atomique des noeuds et des liens.
- [x] Task 3: Script de Migration (AC: 7)
  - [x] Créer `scripts/migrate_memories_v2_to_v3.py`.
  - [x] Parser les anciennes "memories" et les rattacher à `subject:system` ou aux agents via `BELIEVES`.
- [x] Task 4: Validation technique
  - [x] Créer `tests/test_graph_migration.py` pour vérifier l'intégrité du graphe après migration.
...
- Corruption repair pass on surrealdb.py and redis.py.
- Test migration flow validated with mock.

### Completion Notes List

- Implemented SCHEMAFULL Graph Schema in SurrealDB.
- Refactored SurrealDB client for transactional graph inserts.
- Created migration script from v2 (flat) to v3 (graph).
- Validated migration flow with automated tests.

### File List

- apps/h-core/src/infrastructure/surrealdb.py (Modified)
- apps/h-core/src/infrastructure/graph_schema.surql (New)
- scripts/migrate_memories_v2_to_v3.py (New)
- tests/test_graph_migration.py (New)
